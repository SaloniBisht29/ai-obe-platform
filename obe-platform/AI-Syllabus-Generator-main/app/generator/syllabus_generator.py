...
# â”€â”€ Fallback defaults â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def get_default_textbooks(course_name: str) -> list:
    return [
        f"1. Sedra, Adel S. â€” Textbook of {course_name} â€” Oxford University Press â€” 7th Edition â€” 2019",
        f"2. Boylestad, Robert L. â€” Introduction to {course_name} â€” Pearson Education â€” 12th Edition â€” 2018",
        f"3. Floyd, Thomas L. â€” {course_name} Fundamentals â€” Pearson Education â€” 10th Edition â€” 2017",
        f"4. Mano, M. Morris â€” {course_name} Design â€” Pearson Education â€” 5th Edition â€” 2016",
        f"5. Hayt, William H. â€” Engineering {course_name} â€” McGraw Hill â€” 8th Edition â€” 2019",
    ]
...
def get_default_resources() -> dict:
    return {
        "textbooks": [],
        "youtube_resources": [
            "NPTEL â€” Digital Electronics â€” Prof. S. Srinivasan â€” IIT Madras â€” https://nptel.ac.in/courses/",
            "MIT OpenCourseWare â€” Circuit Theory â€” https://ocw.mit.edu/",
            "Khan Academy â€” Electrical Engineering â€” https://www.khanacademy.org/science/electrical-engineering"
        ],
        "open_source_resources": [
            "NPTEL SWAYAM â€” Free Online Courses â€” https://swayam.gov.in",
            "MIT OpenCourseWare â€” Free Course Materials â€” https://ocw.mit.edu",
            "PhET Interactive Simulations â€” University of Colorado â€” https://phet.colorado.edu"
        ],
...
# â”€â”€ Parse functions â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Main 4-call generator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def generate_syllabus(request: SyllabusRequest) -> SyllabusResponse:
    print(f"\n{'='*60}")
    print(f"4-CALL SYLLABUS GENERATION (SEQUENTIAL)")
    ...
    # â”€â”€ CALL 1: Course Objectives + COs â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    course_code, course_objectives, co_objects, co_texts = "", [], [], []
        ...
        try:
            ...
            if request.regenerate:
                prompt1 = build_regenerate_call1_prompt(
                    ...
                    request.num_units, request.education_level or "undergraduate",
                    request.programme or "btech", request.year_of_study, request.semester, request.branch,
                    request.credits or 4, request.ltp or "3:1:0", request.rejection_reason, request.custom_prompt
                )
            else:
                prompt1 = build_call1_prompt(
                    ...
                    request.num_units, request.education_level or "undergraduate",
                    request.programme or "btech", request.year_of_study, request.semester, request.branch,
                    request.credits or 4, request.ltp or "3:1:0", request.custom_prompt
                )
            ...
            if len(co_objects) > 0 and len(course_objectives) > 0:
                print(f"  âœ… Call 1 OK â€” {len(co_objects)} COs | {len(course_objectives)} objectives")
                break
            else:
                print(f"  âš ï¸ Call 1 incomplete (attempt {attempt}) â€” COs:{len(co_objects)} Obj:{len(course_objectives)}")
        except Exception as e:
            print(f"  âŒ Call 1 error (attempt {attempt}): {e}")

    if not course_objectives:
        print("  ðŸ“‹ Using fallback course objectives")
        course_objectives = get_default_objectives(request.course_name)
    if not co_objects:
        print("  ðŸ“‹ Using fallback course outcomes")
    ...
    # â”€â”€ CALL 2: Units â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    units = []
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if request.regenerate:
                prompt2 = build_regenerate_call2_prompt(
                    request.course_name, request.course_description,
                    request.num_units, request.education_level or "undergraduate",
                    request.programme or "btech", request.year_of_study, request.branch,
                    cos_summary, request.rejection_reason, request.custom_prompt
                )
            else:
                prompt2 = build_call2_prompt(
                    ...
                    request.num_units, request.education_level or "undergraduate",
                    request.programme or "btech", request.year_of_study, request.branch,
                    cos_summary, request.custom_prompt
                )
            data2 = call_ollama(prompt2, timeout=300 + (request.num_units * 60))
            units = parse_call2(data2)
            if len(units) > 0:
                print(f"  âœ… Call 2 OK â€” {len(units)} units generated")
                break
            else:
                print(f"  âš ï¸ Call 2 incomplete (attempt {attempt}) â€” 0 units")
        except Exception as e:
            print(f"  âŒ Call 2 error (attempt {attempt}): {e}")

    # â”€â”€ CALL 3: CO-PO Matrix + Attainment â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    matrices = {}
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            print(f"\n[CALL 3/4] Generating CO-PO Matrix + Attainment (attempt {attempt})...")
            prompt3 = build_call3_prompt(request.course_name, cos_summary, request.num_units)
            data3 = call_ollama(prompt3, timeout=300)
            matrices = parse_call3(data3)
            if matrices.get("co_po_matrix"):
                print(f"  âœ… Call 3 OK â€” CO-PO matrix generated")
                break
            else:
                print(f"  âš ï¸ Call 3 incomplete (attempt {attempt})")
        except Exception as e:
            print(f"  âŒ Call 3 error (attempt {attempt}): {e}")

    if not matrices.get("co_po_matrix"):
        print("  ðŸ“‹ Using fallback matrices")
        matrices = get_default_matrices()

    # â”€â”€ CALL 4: Textbooks + Resources + CQI â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    resources = {}
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            prompt4 = build_call4_prompt(
                request.course_name, request.course_description,
                request.education_level or "undergraduate", request.programme or "btech"
            )
            data4 = call_ollama(prompt4, timeout=300)
            resources = parse_call4(data4)
            if resources.get("textbooks"):
                print(f"  âœ… Call 4 OK â€” {len(resources['textbooks'])} textbooks generated")
                break
            else:
                print(f"  âš ï¸ Call 4 incomplete (attempt {attempt})")
        except Exception as e:
            print(f"  âŒ Call 4 error (attempt {attempt}): {e}")

    if not resources.get("textbooks"):
        print("  ðŸ“‹ Using fallback resources")
        resources = get_default_resources()
        resources["textbooks"] = get_default_textbooks(request.course_name)

    # â”€â”€ Assemble final response â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    print(f"\n{'='*60}")
    print(f"4-CALL GENERATION COMPLETE âœ…")
...
def generate_syllabus_streaming(request: SyllabusRequest):
    """
    Generator that yields progress dicts suitable for Server-Sent Events.
    Each dict is JSON-serializable.

    Yields:
        {"step": 1, "total": 4, "label": "...", "done": false}   x4
        {"done": true, "syllabus": { ...SyllabusResponse... }}    final
    """
    ...
    # ── Helpers ──────────────────────────────────────────────────────
    def _progress(step: int, label: str, detail: str = ""):
        return {"step": step, "total": 4, "label": label, "detail": detail, "done": False}
        ...
        try:
            if request.regenerate:
                prompt1 = build_regenerate_call1_prompt(
                    request.course_name, request.course_description,
                    request.num_units, request.education_level or "undergraduate",
                    request.programme or "btech", request.year_of_study, request.semester,
                    request.branch, request.credits or 4, request.ltp or "3:1:0",
    ...
    if not co_objects:
        co_objects = get_default_cos(request.course_name)
        co_texts = [f"{co.co_id}: {co.text}" for co in co_objects]

    cos_summary = build_cos_summary(co_objects)
    yield _progress(1, "Course Objectives + COs", f"{len(co_objects)} COs generated")
    ...
    yield _progress(2, f"{request.num_units} Course Units", f"{len(units)} units generated")
    ...
    yield _progress(3, "CO-PO Matrix + Attainment", "Matrix computed")
    ...
    yield _progress(4, "Textbooks + Resources + CQI", f"{len(resources.get('textbooks', []))} textbooks")

    # ── Assemble + yield final ────────────────────────────────────────
    total_hours    = request.num_units * 8
    total_lectures = request.num_units * 9
    ...
    yield {"done": True, "syllabus": result.dict()}
